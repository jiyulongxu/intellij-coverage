...
public abstract class ExtraFieldInstrumenter extends ClassVisitor {
   ...
   * </pre>
   */
  private final String myFieldInitName;
  ...
  public MethodVisitor createMethodVisitor(final ClassVisitor cv,
    ...
    if (myFieldInitName.equals(name)) return mv;
    ...
    return new MethodVisitor(Opcodes.API_VERSION, newMv) {
      ...
      public void visitCode() {
        if (!myInterface) {
          mv.visitMethodInsn(Opcodes.INVOKESTATIC, myInternalClassName, myFieldInitName, "()V", false);
        }
        super.visitCode();
      }
    };
   ...
   * <pre>
   * <code>
   *   `access` static void {@link ExtraFieldInstrumenter#myFieldInitName}() {
  ...
  private void createInitFieldMethod(ClassVisitor cv) {
    MethodVisitor mv = cv.visitMethod(INIT_METHOD_ACCESS, myFieldInitName, "()V", null, null);
